...
class DDPG(object):
    ...
    def _build_graph(self):
        ...
        self.low_action = tf.constant(self.action_low_bound, shape=[], dtype=tf.float32)
        self.high_action = tf.constant(self.action_high_bound, shape=[], dtype=tf.float32)
        ...
        with tf.variable_scope('actor_loss'):
            self.actor_loss_op = -tf.reduce_mean(self.critic_net)
